...
RESPONSE_STATUS=$(curl --write-out '%{http_code}' --silent --output /dev/null \
		'localhost:4000/v1/users/639686c4ba1604f1387a6c00/inferences/638f56f70acda5864ee0203a' --header "Authorization: Bearer $TOKEN")

FAILED_RESPONSE_STATUS=$(curl --write-out '%{http_code}' --silent --output /dev/null \
		'localhost:4000/v1/users/639686c4ba1604f1387a6c00/inferences/639686c4ba1604f1387a6c00' --header "Authorization: Bearer $TOKEN")

UNAUTH_RESPONSE_STATUS=$(curl --write-out '%{http_code}' --silent --output /dev/null \
		'localhost:4000/v1/users/639686c4ba1604f1387a6c00/inferences/638f56f70acda5864ee0203b' --header "Authorization: Bearer")

FORBID_RESPONSE_STATUS=$(curl --write-out '%{http_code}' --silent --output /dev/null \
		'localhost:4000/v1/users/639686c4ba1604f1387a6c01/inferences/638f56f70acda5864ee0203b' --header "Authorization: Bearer $TOKEN")
